...
//			Destroy(gearShiftingAudio, gearShiftingAudio.GetComponent<AudioSource>().clip.length);
	...
	public void SmokeInstantiateRate()
	{
		...
		if (_wheelParticles.Count > 0)
		{
			...
			var emission0 = _wheelParticles[0].GetComponent<ParticleSystem>().emission;
			emission0.enabled = Mathf.Abs(CorrespondingGroundHit0.sidewaysSlip) > 0.25f || Mathf.Abs(CorrespondingGroundHit0.forwardSlip) > 0.5f;

			FrontLeftWheelCollider.GetGroundHit(out CorrespondingGroundHit1);
			var emission1 = _wheelParticles[1].GetComponent<ParticleSystem>().emission;
			emission1.enabled = Mathf.Abs(CorrespondingGroundHit1.sidewaysSlip) > 0.25f || Mathf.Abs(CorrespondingGroundHit1.forwardSlip) > 0.5f;

			RearRightWheelCollider.GetGroundHit(out CorrespondingGroundHit2);
			var emission2 = _wheelParticles[2].GetComponent<ParticleSystem>().emission;
			emission2.enabled = Mathf.Abs(CorrespondingGroundHit2.sidewaysSlip) > 0.25f || Mathf.Abs(CorrespondingGroundHit2.forwardSlip) > 0.5f;

			RearLeftWheelCollider.GetGroundHit(out CorrespondingGroundHit3);
			var emission3 = _wheelParticles[3].GetComponent<ParticleSystem>().emission;
			emission3.enabled = Mathf.Abs(CorrespondingGroundHit3.sidewaysSlip) > 0.25f || Mathf.Abs(CorrespondingGroundHit3.forwardSlip) > 0.5f;
		}

		if (normalExhaustGas && canControl)
		{
			var normalEmission = normalExhaustGas.emission;
			normalEmission.enabled = speed < 20;
		}

		if (heavyExhaustGas && canControl)
		{
			var heavyEmission = heavyExhaustGas.emission;
			heavyEmission.enabled = speed < 10 && motorInput > 0.1f;
		}
		...
		if (!canControl)
		{
			...
			{
				var heavyEmission = heavyExhaustGas.emission;
				heavyEmission.enabled = false;
			}
			if (normalExhaustGas)
			{
				var normalEmission = normalExhaustGas.emission;
				normalEmission.enabled = false;
			}
		}
	}
